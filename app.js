...
const CATEGORIAS = [
  ...
  { id: "admin-finanzas", nombre: "Administración y Finanzas", emoji: "💳", color: "#C81E8C" },
...
function renderCatnav() {
  ...
  const SHORT_NAMES = {
    ...
    'atencion':     'Atención',
    'administracion': 'Administr.',
    'admin-finanzas': 'Adm. y Finanzas',
    'rrhh':         'RRHH',
    'calidad':      'Calidad',
...
function renderIndex() {
  ...
  grid.innerHTML = CATEGORIAS.map(cat => {
    ...
    return `
      <div class="index-card" data-cat="${cat.id}">
        <div class="index-card-header" style="background:${cat.color};${textColor}">
          <h3>${cat.emoji} ${escHtml(cat.nombre)}</h3>
          <span class="index-card-count">${cursos.length} curso${cursos.length !== 1 ? 's' : ''}</span>
        </div>
        <ul>
          ${cursos.map(c => `
            <li data-anio="${c.anio || ''}"><a href="#${cat.id}">${escHtml(c.titulo)}</a></li>
          `).join('')}
        </ul>
      </div>
    `;
...
function renderCard(c, cat, id) {
  ...
  return `
    <article class="course-card" tabindex="0" role="button" data-curso-id="${id}" data-anio="${c.anio || ''}" aria-label="Ver detalle de ${escAttr(c.titulo)}">
      <div class="course-cover">
        ...
        <div class="course-cat-badge" style="--cat-main:${cat.color};" title="${escAttr(cat.nombre)}" aria-label="Categoría: ${escAttr(cat.nombre)}">${cat.emoji}</div>
      </div>
      <div class="course-body">
        <div class="course-title">${escHtml(c.titulo)}</div>
        <div class="course-bajada">${escHtml(c.bajada)}</div>
        <div class="course-meta">
          ${c.anio ? `<span class="meta-tag anio" title="Año de revisión del contenido"><span class="meta-tag-icon" aria-hidden="true">📅</span> ${c.anio}</span>` : ''}
...
function initSearch() {
  ...
  /* Estado de filtros compartido: texto (buscador) + año (dropdown) */
  let currentQuery = '';
  let currentYear = '';
  ...
  function applyFilters() {
    ...
    document.querySelectorAll('.category-block').forEach(block => {
      ...
      block.querySelectorAll('.course-card').forEach(card => {
        ...
        const matchYear = !currentYear || card.dataset.anio === currentYear;
    ...
    /* El filtro de año también recorta el índice (el texto no toca el índice) */
    document.querySelectorAll('.index-card').forEach(card => {
      let vis = 0;
      card.querySelectorAll('li[data-anio]').forEach(li => {
        const matchYear = !currentYear || li.dataset.anio === currentYear;
        li.classList.toggle('hidden', !matchYear);
        if (matchYear) vis++;
      });
      const countEl = card.querySelector('.index-card-count');
      if (countEl && currentYear) countEl.textContent = `${vis} curso${vis !== 1 ? 's' : ''}`;
      else if (countEl) countEl.textContent = countEl.dataset.full || countEl.textContent;
      card.classList.toggle('hidden', vis === 0);
    });

    if (noResults) noResults.classList.toggle('visible', total === 0);
    applyHighlight(currentQuery.trim());
  }
  ...
  function bindInput(input, other) {
    ...
    input.addEventListener('keydown', e => {
      if (e.key === 'Enter') {
        ...
        clearTimeout(debounceTimer);
        currentQuery = input.value;
        applyFilters();  // inmediato al apretar Enter
  ...
  if (yearSelect) {
    // Guardamos el conteo total original de cada index-card
    document.querySelectorAll('.index-card .index-card-count').forEach(el => {
      el.dataset.full = el.textContent;
    });
    // Poblar opciones con los años presentes, de más nuevo a más viejo
    const years = [...new Set(CURSOS.map(c => c.anio).filter(Boolean))].sort((a, b) => b - a);
    years.forEach(y => {
    ...
    yearSelect.addEventListener('change', () => {
      ...
      yearSelect.classList.toggle('is-active', currentYear !== '');
      applyFilters();
    });
  }
}
